function Encounter_Custom() {
	Encounter_Set(0,-1,battle_enemy,-1,"* You encountered nothing!",-1);

	//Simple Battle
	Encounter_Set(1,-1,battle_enemy_simple,-1,"* You encountered the test&  monster!",-1);

	//Gaster Blaster
	Encounter_Set(2,-1,battle_enemy_gb,-1,"* Gaster Blaster example.",-1);

	//Technoblade
	Encounter_Set(3,-1,battle_enemy_techno,-1,"* Good luck killing him.",-1);

	//Tommy
	Encounter_Set(4,-1,battle_enemy_tommy,-1,"* He said give me a god damn&  minute.",-1);

	//Tubbo
	Encounter_Set(5,-1,battle_enemy_tubbo,-1,"* Tubboat.",-1);


}
